...
class ClubManagerController extends Controller
{
    ...
    public function profile()
    {
        ...
        $stats = [
            ...
            'totalEvents' => Evenement::whereIn('club_id', 
                ClubLecture::where('createur_id', $user->id)->pluck('id'))->count(),
        ];
...
public function indexMembers()
{
    ...
    $members = \App\Models\ClubMember::whereIn('club_id', $clubs->pluck('id'))
        ->with(['user', 'club'])
        ->where('status', 'active')
        ->get();
...
public function removeMember($memberId)
{
    try {
        ...
        $member = \App\Models\ClubMember::where('id', $memberId)
